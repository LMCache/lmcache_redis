...
"""
python benchmark/RESP-py-bench-3.py --pool-size 4096
Running single threaded benchmark
Wrote 16.0 GB in 6.610426902770996 seconds, which is: 2.420418565296143 GB/s
Tested 4096 exists in 0.14792156219482422 seconds
Read 16.0 GB in 4.934222459793091 seconds, which is: 3.2426588242377172 GB/s
Running single threaded benchmark
Wrote 16.0 GB in 6.695870637893677 seconds, which is: 2.38953242457401 GB/s
Tested 4096 exists in 0.1458597183227539 seconds
Read 16.0 GB in 4.971397161483765 seconds, which is: 3.218411138816484 GB/s

python benchmark/RESP-py-bench-3.py --pool-size 4096 --num-threads 1
...
Wrote 16.00 GB in 7.016 s  →  2.281 GB/s
Tested 4096 exists in 0.16693472862243652 seconds
Read  16.00 GB in 5.782 s  →  2.767 GB/s
Running multi threaded benchmark with 4 threads
Wrote 16.00 GB in 4.324 s  →  3.701 GB/s
Tested 4096 exists in 0.1622614860534668 seconds
Read  16.00 GB in 5.184 s  →  3.086 GB/s
Running multi threaded benchmark with 4 threads
Wrote 16.00 GB in 3.916 s  →  4.086 GB/s
Tested 4096 exists in 0.15732526779174805 seconds
Read  16.00 GB in 3.367 s  →  4.752 GB/s
Running multi threaded benchmark with 4 threads
Wrote 16.00 GB in 3.868 s  →  4.137 GB/s
Tested 4096 exists in 0.14825677871704102 seconds
Read  16.00 GB in 3.411 s  →  4.690 GB/s

python benchmark/RESP-py-bench-3.py --pool-size 4096 --num-threads 8
Running multi threaded benchmark with 8 threads
Wrote 16.00 GB in 3.408 s  →  4.695 GB/s
Tested 4096 exists in 0.13771557807922363 seconds
Read  16.00 GB in 2.705 s  →  5.914 GB/s
Running multi threaded benchmark with 8 threads
Wrote 16.00 GB in 3.424 s  →  4.673 GB/s
Tested 4096 exists in 0.12969470024108887 seconds
Read  16.00 GB in 2.477 s  →  6.461 GB/s
Running multi threaded benchmark with 8 threads
Wrote 16.00 GB in 3.313 s  →  4.830 GB/s
Tested 4096 exists in 0.13681340217590332 seconds
Read  16.00 GB in 2.971 s  →  5.385 GB/s

python benchmark/RESP-py-bench-3.py --pool-size 4096 --num-threads 8
Running multi threaded benchmark with 16 threads
Wrote 16.00 GB in 3.572 s  →  4.480 GB/s
Tested 4096 exists in 0.13534212112426758 seconds
Read  16.00 GB in 3.490 s  →  4.584 GB/s
Running multi threaded benchmark with 16 threads
Wrote 16.00 GB in 3.440 s  →  4.652 GB/s
Tested 4096 exists in 0.13901162147521973 seconds
Read  16.00 GB in 3.661 s  →  4.371 GB/s
"""
...
class MultiRESPClient:
    ...
    Presents an async interface
    """

    def __init__(self, host: str, port: int, chunk_size: int, num_threads: int, loop: asyncio.AbstractEventLoop):
        """
        Accept the loop manually
        """
        self.loop = loop
        ...
        while True:
            ...
            try:
                ...
                if op == "get":
                    ...
                    self.loop.call_soon_threadsafe(future.set_result, exists)
    ...
    async def aclose(self):
        ...
        await asyncio.gather(*(asyncio.to_thread(t.join) for t in self.threads))
...
async def benchmark_write_concurrent(client: MultiRESPClient, pool: BufferPool):
    start_time = time.time()
    futures = []
    for i in range(pool.pool_size):
        buf = pool.pool[i]
...
async def benchmark_exists_concurrent(client: MultiRESPClient, pool: BufferPool):
    start_time = time.time()
    futures = []
    for i in range(pool.pool_size):
        ...
        futures.append(fut)
    for fut in futures:
        assert await fut, f"chunk_{i} does not exist after write"
    end_time = time.time()
    print(f"Tested {pool.pool_size} exists in {end_time - start_time} seconds")

async def benchmark_read_concurrent(client: MultiRESPClient, pool: BufferPool):
    start_time = time.time()
    futures = []
    for i in range(pool.pool_size):
        buf = pool.pool[i]
...
if __name__ == "__main__":
    ...
    # multi threaded codepath
        ...
        async def async_main():
            ...
            client = MultiRESPClient(host="localhost", port=6379, chunk_size=args.buffer_size, num_threads=args.num_threads, loop=loop)
            await benchmark_write_concurrent(client, pool)
            await benchmark_exists_concurrent(client, pool)
            await benchmark_read_concurrent(client, pool)
            await client.aclose()
